...
public class Tests {
    ...
    public void getCurrentMaxTest(){
        ...
        Future<Integer>[] sumTask = new Future[1000];
        for (int i = 0; i < 1000; i++) {
            var task = Task.createTask(mission, TaskType.COMPUTATIONAL);
            //System.out.println(i);
            sumTask[i] = customExecutor.submit(task);
        ...
        final int sum;
        try {
            sum = sumTask[0].get(1, TimeUnit.MILLISECONDS);
        } catch (InterruptedException | ExecutionException | TimeoutException e) {
            throw new RuntimeException(e);
        }
        logger.info(()-> "Sum of 1 through 10 = " + sum);
